...
class Magmodules_Webwinkelconnect_Block_Adminhtml_Widget_Grid_Log
    extends Mage_Adminhtml_Block_Widget_Grid_Column_Renderer_Action
{
    ...
    public function render(Varien_Object $row)
    {
        ...
        if ($type == 'invitation') {
            if ($row->getOrderId()) {
                $incrementId = Mage::getResourceModel('sales/order')->getIncrementId($row->getOrderId());
                $orderUrl = Mage::helper('adminhtml')->getUrl(
                    "adminhtml/sales_order/view",
                    array('order_id'=> $row->getOrderId())
                );
                $orderUrlText = '<a href="' . $orderUrl . '">#' . $incrementId .'</a>';
                $msg = Mage::helper('webwinkelconnect')->__('%s - Repsonse: %s', $orderUrlText, $row->getResponse());
            }    
        }
